# pitchkit/pipeline — feature registry, runner, default graph, CLI
...
- **`cli.py`** — `python -m pitchkit.pipeline.cli <video> [--feature NAME] [--max-frames N]`.
...
```
```bash
python -m pitchkit.pipeline.cli football-1.mp4 --max-frames 50             # full run
python -m pitchkit.pipeline.cli football-1.mp4 --feature detect            # isolate
```
...
## Original reference
Replaces the orchestration in `src/graph/` (`match_graph.py`, `nodes.py`) + `run_pipeline.py`.
