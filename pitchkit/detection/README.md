# pitchkit/detection — per-frame detection (pipeline entry point)
...
```
CLI: `python -m pitchkit.pipeline.cli football-1.mp4 --feature detect --max-frames 50`
...
## Original reference
Ported from `src/agents/vision/vision_agent.py` (detection half only).
...
```python
from pitchkit.detection.detect import detect_ball_sahi
