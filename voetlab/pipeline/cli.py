"""CLI: ``python -m voetlab.pipeline.cli <video> [--feature NAME] [--max-frames N]``.

Runs the full pipeline by default, or a single feature in isolation with ``--feature``.
Dumps a ``results.json`` summary to ``voetlab/tests/out/<tag>/`` for inspection.
"""
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="voetlab")
    ...
    p.add_argument("--out", default=None, help="artifact output dir (default: voetlab/tests/out)")
    ...
    print(f"[voetlab] {tag}: ok={res.ok} -> {out_dir / 'results.json'}")

    if not res.ok:
        print(f"[voetlab] FAILED: {res.error}", file=sys.stderr)
        return 1
    return 0
